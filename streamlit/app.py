...
# ----------------------- PAGE: Occupancy Prediction (ADDED) -----------------------
    ...
    direction_id = st.selectbox("Direction", [0, 1], index=0)  # kept for parity with your snippet
    ...
    # Step 1: fetch route center coords to derive bins (same logic as training)
    SQL_FETCH_COORDS = f"""
    SELECT
    ...
    if not coords.empty and not coords.isnull().values.any():
        ...
        # Step 2: bins (adjust if your training used a different binning rule)
        ...
        with st.spinner("Querying BigQuery ML model..."):
            try:
                ...
                if not df_pred.empty:
                    ...
                    # Try to render class probabilities if present
                    probs = df_pred.iloc[0].get("predicted_label_probs", None)
                    if probs is not None:
                        # BigQuery returns ARRAY<STRUCT<label STRING, prob FLOAT64>> or similar
                        try:
